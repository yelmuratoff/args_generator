...
class TestPageArgs {
  ...
  static TestPageArgs? tryParse(Map<String, String> args) {
    try {
      return TestPageArgs(
        ...
        dateTimeValue: DateTime.tryParse(args['date-time-value'] ?? '') ?? DateTime.fromMillisecondsSinceEpoch(0),
        ...
        iterableValue:
            args.containsKey('iterable-value') ? args['iterable-value']!.split(',').map((e) => e).toList() : [],
        ...
        optionalIterable:
            args.containsKey('optional-iterable') ? args['optional-iterable']!.split(',').map((e) => e).toList() : null,
        ...
        defType: args.containsKey('def-type')
            ? TestEnum.values.where((e) => e.toString().split('.').last == args['def-type']).firstOrNull
            : null,
        defaultInt: int.tryParse(args['default-int'] ?? '0'),
        defaultNum: num.tryParse(args['default-num'] ?? '0'),
        defaultString: args['default-string'] ?? '',
        defaultIterable:
            args.containsKey('default-iterable') ? args['default-iterable']!.split(',').map((e) => e).toList() : null,
      );
    } catch (e) {
      return null;
    }
  ...
  static const _$TestEnumEnumMap = {TestEnum.value1: 'value1', TestEnum.value2: 'value2', TestEnum.value3: 'value3'};
}
